...
const skillBaseData = {
	...
	'Void Amplification': {
		...
		maxXp: 100,
		effect: 0.01,
		description: 'The Void XP',
	},
	'Mind Seize': {
		name: 'Mind Seize',
		maxXp: 100,
		effect: 0.0006,
		description: 'Reduced Happiness',
	},
	'Ceaseless Abyss': {
		name: 'Ceaseless Abyss',
		maxXp: 100,
		effect: 0.000585,
		description: 'Longer Lifespan',
	},
	...
	'Cosmic Longevity': {
		name: 'Cosmic Longevity',
		maxXp: 100,
		effect: 0.0015,
		description: 'Longer Lifespan',
	},
	'Cosmic Recollection': {
		name: 'Cosmic Recollection',
		maxXp: 100,
		effect: 0.00065,
		description: 'Max Lvl Multiplier',
	},
	'Essence Collector': {
		name: 'Essence Collector',
		maxXp: 100,
		effect: 0.01,
		description: 'Essence Gain',
	},
	'Galactic Command': {
	...
	'Yin Yang': { name: 'Yin Yang', maxXp: 100, effect: 0.02, description: 'Essence + Evil Gain' },
	'Parallel Universe': {
...
const itemBaseData = {
	...
	"Mind's Eye": {
		...
		expense: 3250000000,
		effect: 10,
		description: 'Fundamentals XP',
	},
	'Void Necklace': {
		name: 'Void Necklace',
		expense: 28050000000,
		effect: 3,
		description: 'Void Manipulation XP',
	},
	'Void Armor': {
		name: 'Void Armor',
		expense: 197050000000,
		effect: 3,
		description: 'The Void XP',
	},
	...
	'Void Orb': {
		name: 'Void Orb',
		expense: 1202050000000,
		effect: 3,
		description: 'Void Manipulation XP',
	},
	'Void Dust': {
		name: 'Void Dust',
		expense: 25002050000000,
		effect: 3,
		description: 'The Void XP',
	},
	'Celestial Robe': {
		name: 'Celestial Robe',
		expense: 300002050000000,
		effect: 5,
		description: 'Galactic Council XP',
	},
	'Universe Fragment': {
		name: 'Universe Fragment',
		expense: 18500002050000000,
		effect: 3,
		description: 'Ability XP',
	},
	'Multiverse Fragment': {
		name: 'Multiverse Fragment',
		expense: 200500002050000000,
		effect: 5,
...
function getHappiness() {
	...
	var happiness =
		((meditationEffect() * butlerEffect()) / mindseizeEffect()) *
		multiverseFragment() *
...
function getEvilGain() {
	...
	var evil =
		evilControl.getEffect() *
		bloodMeditation.getEffect() *
		absoluteWish.getEffect() *
		oblivionEmbodiment.getEffect() *
		yingYang.getEffect()
	return evil
}
...
function getGameSpeed() {
	...
	var timeWarpingSpeed = gameData.timeWarpingEnabled
		? timeWarping.getEffect() + temporalDimension.getEffect() * timeLoop.getEffect()
		: 1
...
function createAllRows(categoryType, tableId) {
	var templates = {
		headerRow: document.getElementsByClassName(
			categoryType == itemCategories ? 'headerRowItemTemplate' : 'headerRowTaskTemplate'
		)[0],
		row: document.getElementsByClassName(
			categoryType == itemCategories ? 'rowItemTemplate' : 'rowTaskTemplate'
		)[0],
	}
...
function updateItemRows() {
	for (key in gameData.itemData) {
		...
		active.style.backgroundColor =
			gameData.currentMisc.includes(item) || item == gameData.currentProperty ? color : 'white'
...
function updateHeaderRows(categories) {
	for (categoryName in categories) {
		...
		skipSkillElement.style.display =
			categories == skillCategories && autoLearnElement.checked ? 'block' : 'none'
	}
}
...
function updateText() {
	...
	document.getElementById('timeWarpingButton').textContent = gameData.timeWarpingEnabled
		? 'Disable warp'
		: 'Enable warp'
}
...
function setSkillWithLowestMaxXp() {
	...
	enabledSkills.sort((lhs, rhs) => {
		return lhs.getMaxXp() / lhs.getXpGain() - rhs.getMaxXp() / rhs.getXpGain()
	})
...
function formatCoins(coins, element) {
	...
	if ((leftOver == 0 && coins > 0) || coins > 1e9) {
	...
	for (key in gameData.requirements) {
		...
		if (requirement.type == 'task') {
			requirement = Object.assign(
				new TaskRequirement(requirement.elements, requirement.requirements),
				requirement
			)
		} else if (requirement.type == 'coins') {
			requirement = Object.assign(
				new CoinRequirement(requirement.elements, requirement.requirements),
				requirement
			)
		} else if (requirement.type == 'age') {
			requirement = Object.assign(
				new AgeRequirement(requirement.elements, requirement.requirements),
				requirement
			)
		} else if (requirement.type == 'evil') {
			requirement = Object.assign(
				new EvilRequirement(requirement.elements, requirement.requirements),
				requirement
			)
		} else if (requirement.type == 'essence') {
			requirement = Object.assign(
				new EssenceRequirement(requirement.elements, requirement.requirements),
				requirement
			)
		}
...
window.addEventListener('keydown', function (e) {
	...
	if (e.key == 'l' || e.key == 'L')
		document.getElementById('autoLearn').checked = !document.getElementById('autoLearn').checked
	if (e.key == 'p' || e.key == 'P')
...
gameData.requirements = {
	...
	'Galactic Council': new AgeRequirement(getElementsByClass('Galactic Council'), [
		{ requirement: 10000 },
	]),
	'The Void': new AgeRequirement(getElementsByClass('The Void'), [{ requirement: 1000 }]),
	'Void Manipulation': new AgeRequirement(getElementsByClass('Void Manipulation'), [
		{ requirement: 1000 },
	]),
	'Celestial Powers': new AgeRequirement(getElementsByClass('Celestial Powers'), [
		{ requirement: 10000 },
	]),
	'Dark Magic': new EvilRequirement(getElementsByClass('Dark Magic'), [{ requirement: 1 }]),
	Almightiness: new EssenceRequirement(getElementsByClass('Almightiness'), [{ requirement: 1 }]),
	Shop: new CoinRequirement(
		[document.getElementById('shopTabButton')],
		[{ requirement: gameData.itemData['Tent'].getExpense() * 50 }]
	),
	'Rebirth tab': new AgeRequirement(
		[document.getElementById('rebirthTabButton')],
		[{ requirement: 25 }]
	),
	'Rebirth note 1': new AgeRequirement(
		[document.getElementById('rebirthNote1')],
		[{ requirement: 45 }]
	),
	'Rebirth note 2': new AgeRequirement(
		[document.getElementById('rebirthNote2')],
		[{ requirement: 65 }]
	),
	'Rebirth note 3': new AgeRequirement(
		[document.getElementById('rebirthNote3')],
		[{ requirement: 200 }]
	),
	'Rebirth note 4': new AgeRequirement(
		[document.getElementById('rebirthNote4')],
		[{ requirement: 1000 }]
	),
	'Rebirth note 4': new AgeRequirement(
		[document.getElementById('rebirthNote4')],
	...
	'Rebirth note 5': new AgeRequirement(
		[document.getElementById('rebirthNote5')],
		[{ requirement: 10000 }]
	),
	'Rebirth note 6': new TaskRequirement(
		[document.getElementById('rebirthNote6')],
		[{ task: 'Cosmic Recollection', requirement: 1 }]
	),
	'Evil info': new EvilRequirement([document.getElementById('evilInfo')], [{ requirement: 1 }]),
	'Essence info': new EssenceRequirement(
		[document.getElementById('essenceInfo')],
		[{ requirement: 1 }]
	),
	'Time warping info': new TaskRequirement(
		[document.getElementById('timeWarping')],
		[{ task: 'Adept Mage', requirement: 10 }]
	),
	...
	Fisherman: new TaskRequirement(
		[getTaskElement('Fisherman')],
		[{ task: 'Farmer', requirement: 10 }]
	),
	...
	Centenary: new TaskRequirement(
		[getTaskElement('Centenary')],
		[
	...
	'Lieutenant General': new TaskRequirement(
		[getTaskElement('Lieutenant General')],
		[
	...
	'Adept Mage': new TaskRequirement(
		[getTaskElement('Adept Mage')],
		[
	...
	Archmage: new TaskRequirement(
		[getTaskElement('Archmage')],
		[
			{ task: 'Mana Control', requirement: 1200 },
			{ task: 'Master Wizard', requirement: 10 },
		]
	),
	Chronomancer: new TaskRequirement(
		[getTaskElement('Chronomancer')],
		[
			{ task: 'Mana Control', requirement: 1500 },
			{ task: 'Meditation', requirement: 1500 },
			{ task: 'Archmage', requirement: 25 },
		]
	),
	Chairman: new TaskRequirement(
		...
		[
			...
			{ task: 'Productivity', requirement: 2000 },
			{ task: 'Chronomancer', requirement: 50 },
		]
	),
	Imperator: new TaskRequirement(
		[getTaskElement('Imperator')],
		[
			{ task: 'All Seeing Eye', requirement: 3000 },
			{ task: 'Concentration', requirement: 3000 },
			{ task: 'Chairman', requirement: 666 },
		]
	),

	//The Void
	Corrupted: new AgeRequirement(
		[getTaskElement('Corrupted')],
		[{ task: 'Squire', requirement: 1000 }]
	),
	'Void Slave': new TaskRequirement(
		[getTaskElement('Void Slave')],
		[{ task: 'Corrupted', requirement: 30 }]
	),
	'Void Fiend': new TaskRequirement(
		[getTaskElement('Void Fiend')],
		[
			{ task: 'Brainwashing', requirement: 3000 },
			{ task: 'Void Slave', requirement: 200 },
		]
	),
	'Abyss Anomaly': new TaskRequirement(
		[getTaskElement('Abyss Anomaly')],
		[
			{ task: 'Mind Seize', requirement: 3000 },
			{ task: 'Void Fiend', requirement: 200 },
		]
	),
	'Void Wraith': new TaskRequirement(
		[getTaskElement('Void Wraith')],
		[
			{ task: 'Temporal Dimension', requirement: 3400 },
			{ task: 'Abyss Anomaly', requirement: 300 },
		]
	),
	'Void Reaver': new TaskRequirement(
		[getTaskElement('Void Reaver')],
		[
			{ task: 'Void Amplification', requirement: 3400 },
			{ task: 'Void Wraith', requirement: 250 },
		]
	),
	'Void Lord': new TaskRequirement(
		[getTaskElement('Void Lord')],
		[
			{ task: 'Void Symbiosis', requirement: 3800 },
			{ task: 'Void Reaver', requirement: 150 },
		]
	),
	'Abyss God': new TaskRequirement(
		[getTaskElement('Abyss God')],
		[
			{ task: 'Void Embodiment', requirement: 4700 },
			{ task: 'Void Lord', requirement: 750 },
		]
	),

	//Galactic Council
	'Eternal Wanderer': new AgeRequirement(
		[getTaskElement('Eternal Wanderer')],
		[{ task: 'Squire', requirement: 10000 }]
	),
	Nova: new TaskRequirement(
		[getTaskElement('Nova')],
		[
			{ task: 'Eternal Wanderer', requirement: 15 },
			{ task: 'Cosmic Longevity', requirement: 4000 },
		]
	),
	'Sigma Proioxis': new TaskRequirement(
		[getTaskElement('Sigma Proioxis')],
		[
			{ task: 'Nova', requirement: 200 },
			{ task: 'Cosmic Recollection', requirement: 4500 },
		]
	),
	Acallaris: new TaskRequirement(
		[getTaskElement('Acallaris')],
		[
			{ task: 'Galactic Command', requirement: 5000 },
			{ task: 'Sigma Proioxis', requirement: 1000 },
		]
	),
	'One Above All': new TaskRequirement(
		[getTaskElement('One Above All')],
		[
			{ task: 'Meditation', requirement: 6300 },
			{ task: 'Acallaris', requirement: 1400 },
		]
	),
	...
	Productivity: new TaskRequirement(
		[getTaskElement('Productivity')],
		[{ task: 'Concentration', requirement: 5 }]
	),
	Bargaining: new TaskRequirement(
		[getTaskElement('Bargaining')],
	...
	'Life Essence': new TaskRequirement(
		[getTaskElement('Life Essence')],
		[{ task: 'Apprentice Mage', requirement: 10 }]
	),
	'Time Warping': new TaskRequirement(
		[getTaskElement('Time Warping')],
		[{ task: 'Adept Mage', requirement: 10 }]
	),
	'Astral Body': new TaskRequirement(
		[getTaskElement('Astral Body')],
		[{ task: 'Archmage', requirement: 10 }]
	),
	'Temporal Dimension': new TaskRequirement(
		[getTaskElement('Temporal Dimension')],
		[{ task: 'Chronomancer', requirement: 25 }]
	),
	'All Seeing Eye': new TaskRequirement(
		[getTaskElement('All Seeing Eye')],
		[
			{ task: 'Mana Control', requirement: 2500 },
			{ task: 'Chairman', requirement: 100 },
		]
	),
	Brainwashing: new TaskRequirement(
		[getTaskElement('Brainwashing')],
		[{ task: 'Imperator', requirement: 100 }]
	),
	...
	'Blood Meditation': new EvilRequirement(
		[getTaskElement('Blood Meditation')],
		[{ requirement: 50 }]
	),
	"Demon's Wealth": new EvilRequirement([getTaskElement("Demon's Wealth")], [{ requirement: 500 }]),
	'Dark Knowledge': new EvilRequirement(
		[getTaskElement('Dark Knowledge')],
		[{ requirement: 5000 }]
	),
	'Void Influence': new EvilRequirement(
		[getTaskElement('Void Influence')],
		[{ requirement: 50000 }]
	),
	'Time Loop': new EvilRequirement([getTaskElement('Time Loop')], [{ requirement: 2500000 }]),
	'Evil Incarnate': new EvilRequirement(
		[getTaskElement('Evil Incarnate')],
		[{ requirement: 1000000000 }]
	),

	//Void Manipulation
	'Absolute Wish': new TaskRequirement(
		[getTaskElement('Absolute Wish')],
		[
			{ task: 'Void Slave', requirement: 25 },
			{ task: 'Chairman', requirement: 300 },
		]
	),
	'Void Amplification': new TaskRequirement(
		[getTaskElement('Void Amplification')],
		[
			{ task: 'Void Slave', requirement: 100 },
			{ task: 'Absolute Wish', requirement: 3000 },
		]
	),
	'Mind Seize': new TaskRequirement(
		[getTaskElement('Mind Seize')],
		[{ task: 'Void Amplification', requirement: 3000 }]
	),
	'Ceaseless Abyss': new TaskRequirement(
		[getTaskElement('Ceaseless Abyss')],
		[
			{ task: 'Void Influence', requirement: 4000 },
			{ task: 'Abyss Anomaly', requirement: 50 },
		]
	),
	'Void Symbiosis': new TaskRequirement(
		[getTaskElement('Void Symbiosis')],
		[
			{ task: 'Ceaseless Abyss', requirement: 3500 },
			{ task: 'Void Reaver', requirement: 50 },
		]
	),
	'Void Embodiment': new TaskRequirement(
		[getTaskElement('Void Embodiment')],
		[
			{ task: 'Dark Influence', requirement: 4600 },
			{ task: 'Void Lord', requirement: 50 },
		]
	),
	'Abyss Manipulation': new TaskRequirement(
		[getTaskElement('Abyss Manipulation')],
		[
			{ task: 'Abyss God', requirement: 350 },
			{ task: 'Dark Influence', requirement: 6000 },
			{ task: 'Void Influence', requirement: 6000 },
		]
	),

	//Celestial Powers
	'Cosmic Longevity': new TaskRequirement(
		[getTaskElement('Cosmic Longevity')],
		[{ task: 'Eternal Wanderer', requirement: 1 }]
	),
	'Cosmic Recollection': new TaskRequirement(
		[getTaskElement('Cosmic Recollection')],
		[
			{ task: 'Nova', requirement: 50 },
			{ task: 'Meditation', requirement: 4200 },
			{ task: 'Mind Seize', requirement: 3000 },
		]
	),
	'Essence Collector': new TaskRequirement(
		[getTaskElement('Essence Collector')],
		[
			{ task: 'Sigma Proioxis', requirement: 500 },
			{ task: 'Absolute Wish', requirement: 4900 },
			{ task: 'Dark Knowledge', requirement: 6300 },
		]
	),
	'Galactic Command': new TaskRequirement(
		[getTaskElement('Galactic Command')],
		[
			{ task: 'Essence Collector', requirement: 5000 },
			{ task: 'Bargaining', requirement: 5000 },
		]
	),

	//Essence
	'Yin Yang': new EssenceRequirement([getTaskElement('Yin Yang')], [{ requirement: 1 }]),
	'Parallel Universe': new EssenceRequirement(
		[getTaskElement('Parallel Universe')],
		[{ requirement: 1 }]
	),
	'Higher Dimensions': new EssenceRequirement(
		[getTaskElement('Higher Dimensions')],
		[{ requirement: 10000 }]
	),
	...
	'Wooden Hut': new CoinRequirement(
		[getItemElement('Wooden Hut')],
		[{ requirement: gameData.itemData['Wooden Hut'].getExpense() * 100 }]
	),
	Cottage: new CoinRequirement(
		[getItemElement('Cottage')],
		[{ requirement: gameData.itemData['Cottage'].getExpense() * 100 }]
	),
	House: new CoinRequirement(
		[getItemElement('House')],
		[{ requirement: gameData.itemData['House'].getExpense() * 100 }]
	),
	'Large House': new CoinRequirement(
		[getItemElement('Large House')],
		[{ requirement: gameData.itemData['Large House'].getExpense() * 100 }]
	),
	'Small Palace': new CoinRequirement(
		[getItemElement('Small Palace')],
		[{ requirement: gameData.itemData['Small Palace'].getExpense() * 100 }]
	),
	'Grand Palace': new CoinRequirement(
		[getItemElement('Grand Palace')],
		[{ requirement: gameData.itemData['Grand Palace'].getExpense() * 100 }]
	),
	'Town Ruler': new CoinRequirement(
		[getItemElement('Town Ruler')],
		[{ requirement: gameData.itemData['Town Ruler'].getExpense() * 100 }]
	),
	'City Ruler': new CoinRequirement(
		[getItemElement('City Ruler')],
		[{ requirement: gameData.itemData['City Ruler'].getExpense() * 100 }]
	),
	'Nation Ruler': new CoinRequirement(
		[getItemElement('Nation Ruler')],
		[{ requirement: gameData.itemData['Nation Ruler'].getExpense() * 100 }]
	),
	'Pocket Dimension': new CoinRequirement(
		[getItemElement('Pocket Dimension')],
		[{ requirement: gameData.itemData['Pocket Dimension'].getExpense() * 100 }]
	),
	'Void Realm': new CoinRequirement(
		[getItemElement('Void Realm')],
		[{ requirement: gameData.itemData['Void Realm'].getExpense() * 100 }]
	),
	'Void Universe': new CoinRequirement(
		[getItemElement('Void Universe')],
		[{ requirement: gameData.itemData['Void Universe'].getExpense() * 100 }]
	),
	'Astral Realm': new CoinRequirement(
		[getItemElement('Astral Realm')],
		[{ requirement: gameData.itemData['Astral Realm'].getExpense() * 100 }]
	),
	'Galactic Throne': new CoinRequirement(
		[getItemElement('Galactic Throne')],
		[{ requirement: gameData.itemData['Galactic Throne'].getExpense() * 100 }]
	),

	//Misc
	Book: new CoinRequirement([getItemElement('Book')], [{ requirement: 0 }]),
	Dumbbells: new CoinRequirement(
		[getItemElement('Dumbbells')],
		[{ requirement: gameData.itemData['Dumbbells'].getExpense() * 100 }]
	),
	'Personal Squire': new CoinRequirement(
		[getItemElement('Personal Squire')],
		[{ requirement: gameData.itemData['Personal Squire'].getExpense() * 100 }]
	),
	'Steel Longsword': new CoinRequirement(
		[getItemElement('Steel Longsword')],
		[{ requirement: gameData.itemData['Steel Longsword'].getExpense() * 100 }]
	),
	Butler: new CoinRequirement(
		[getItemElement('Butler')],
		[{ requirement: gameData.itemData['Butler'].getExpense() * 100 }]
	),
	'Sapphire Charm': new CoinRequirement(
		[getItemElement('Sapphire Charm')],
		[{ requirement: gameData.itemData['Sapphire Charm'].getExpense() * 100 }]
	),
	'Study Desk': new CoinRequirement(
		[getItemElement('Study Desk')],
		[{ requirement: gameData.itemData['Study Desk'].getExpense() * 100 }]
	),
	Library: new CoinRequirement(
		[getItemElement('Library')],
		[{ requirement: gameData.itemData['Library'].getExpense() * 100 }]
	),
	Observatory: new CoinRequirement(
		[getItemElement('Observatory')],
		[{ requirement: gameData.itemData['Observatory'].getExpense() * 100 }]
	),
	"Mind's Eye": new CoinRequirement(
		[getItemElement("Mind's Eye")],
		[{ requirement: gameData.itemData["Mind's Eye"].getExpense() * 100 }]
	),
	'Void Necklace': new CoinRequirement(
		[getItemElement('Void Necklace')],
		[{ requirement: gameData.itemData['Void Necklace'].getExpense() * 100 }]
	),
	'Void Armor': new CoinRequirement(
		[getItemElement('Void Armor')],
		[{ requirement: gameData.itemData['Void Armor'].getExpense() * 100 }]
	),
	'Void Blade': new CoinRequirement(
		[getItemElement('Void Blade')],
		[{ requirement: gameData.itemData['Void Blade'].getExpense() * 100 }]
	),
	'Void Orb': new CoinRequirement(
		[getItemElement('Void Orb')],
		[{ requirement: gameData.itemData['Void Orb'].getExpense() * 100 }]
	),
	'Void Dust': new CoinRequirement(
		[getItemElement('Void Dust')],
		[{ requirement: gameData.itemData['Void Dust'].getExpense() * 100 }]
	),
	'Celestial Robe': new CoinRequirement(
		[getItemElement('Celestial Robe')],
		[{ requirement: gameData.itemData['Celestial Robe'].getExpense() * 100 }]
	),
	'Universe Fragment': new CoinRequirement(
		[getItemElement('Universe Fragment')],
		[{ requirement: gameData.itemData['Universe Fragment'].getExpense() * 100 }]
	),
	'Multiverse Fragment': new CoinRequirement(
		[getItemElement('Multiverse Fragment')],
		[{ requirement: gameData.itemData['Multiverse Fragment'].getExpense() * 100 }]
	),
}
